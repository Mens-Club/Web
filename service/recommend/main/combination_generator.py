...
def generate_proper_combinations(recommend_json, items_by_category, styles, max_count=3):
        ...
        for category, subcategories in recommend_json.items():
            ...
            if category == "하의" and "숏팬츠" in subcategories:
                ...
                if shorts_items:
                    # 이미 사용하지 않은 아이템 필터링
                    available_items = [
                        item for item in shorts_items 
                        if item.get('idx') not in used_items[category]
                    ]
                    
                    if available_items:
                        selected_item = random.choice(available_items)
                        combo[category] = selected_item
                        used_items[category].append(selected_item.get('idx'))
            ...
            if category == "신발":
                ...
                if sub in items_by_category.get(category, {}):
                    for item in items_by_category[category][sub]:
                        if item.get("style") == current_style and item.get('idx') not in used_items[category]:
                            matching_items.append(item)
                
                # 스타일 제한 없이 찾기
                if not matching_items and sub in items_by_category.get(category, {}):
                    matching_items = [
                        item for item in items_by_category[category][sub]
                        if item.get('idx') not in used_items[category]
                    ]
                ...
                if matching_items:
                    ...
                    combo[category] = selected_item
                    used_items[category].append(selected_item.get('idx'))
                elif sub in items_by_category.get(category, {}) and items_by_category[category][sub]:
                    # 중복 허용
