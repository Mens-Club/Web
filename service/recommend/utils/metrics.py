...
def push_fashion_recommendation_metrics(success: bool, duration: float):
    ...
    g_success = Gauge(
        "fashion_recommend_success",
        "패션 추천 요청의 성공 여부 (1: 성공, 0: 실패)",
        registry=registry,
    )
    g_duration = Gauge(
        "fashion_recommend_duration_seconds",
        "패션 추천 처리에 걸린 시간(초)",
        registry=registry,
    )
    g_timestamp = Gauge(
        "fashion_recommend_last_run_timestamp",
        "마지막 패션 추천 실행 시간 (UNIX timestamp)",
        registry=registry,
    )
    ...
    try:
        push_to_gateway(
            PUSH_GATEWAY_URL, job="fashion_recommend_api", registry=registry
        )
...
def fail_response(
    message: str, start_time: float, status: int = 500, extra: dict = None
):
