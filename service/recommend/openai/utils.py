...
def generate_reasoning_task(
    recommendation_ids, combinations, season, styles, original_item_info=None
):

    try:
        for id, recommendation_id in enumerate(recommendation_ids):
            combo = combinations[id]
            style = styles[id % len(styles)]
