...
from .serializers import LikeSerializer, MainLikeSerializer, MainTableSerializer, MainPickedSerializer, CombinedPickSerializer
from .models import MainTable, MainPicked, Picked
...
def _build_combined_list(user_id):
    ...
    rec_qs = Picked.objects.filter(user_id=user_id).select_related('recommendation')
    rec_list = [{
        'source': 'recommend',
        'pick_id': p.id,
        'created_at': p.created_at,
        'combination': {
            **MainTableSerializer(p.recommendation).data  # adapt if needed
        }
    } for p in rec_qs]

    # main_table 기반 MainPicked
    main_qs = MainPicked.objects.filter(user_id=user_id).select_related('main')
    main_list = [{
        'source': 'main',
        'pick_id': p.id,
        'created_at': p.created_at,
        'combination': MainTableSerializer(p.main).data
    } for p in main_qs]
...
class MainRandomAPIView(APIView):
    ...
    def get(self, request):
        ...
        items = MainTable.objects.order_by('?')[:count]
...
class MainByPriceAPIView(APIView):
    ...
    def get(self, request):
        ...
        for b in bracket_values:
            items = MainTable.objects.filter(
                total_price__gte=b,
...
class MainByStyleAPIView(APIView):
    ...
    def get(self, request):
        ...
        count = int(request.query_params.get('count'))
        items = MainTable.objects.filter(style=style).order_by('?')[:count]
        return Response(MainTableSerializer(items, many=True).data)
