...
class PickedClothesLikeCancelView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="사용자가 좋아요를 취소한 옷을 picked_clothes에서 UUID로 삭제합니다.",
        manual_parameters=[openapi.Parameter('uuid', openapi.IN_QUERY, description="삭제할 옷의 UUID", type=openapi.TYPE_STRING)],
        responses={
            200: openapi.Response(
                description="좋아요 취소된 옷을 UUID로 삭제한 성공",
                schema=openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    properties={
    ...
    def delete(self, request):
        ...
        try:
            # UUID로 해당 문서 조회
            res = es.search(index="picked_clothes", body={
                "query": {
                    "term": {"_id": uuid}  # _id로 문서 조회
                }
            })

            if res["hits"]["hits"]:
                # 문서가 존재하면 삭제
                es.delete(index="picked_clothes", id=uuid)  # _id를 사용해 삭제
                # 삭제 후 즉시 반영되도록 refresh 호출
                es.indices.refresh(index="picked_clothes")
                return Response({"message": "Clothing removed from favorites successfully."}, status=status.HTTP_200_OK)
            else:
